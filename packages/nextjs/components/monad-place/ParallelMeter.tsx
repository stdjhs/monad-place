...
// 配色遵循 dataviz 规范：紫=本合约结构 / 绿=整链性能，无渐变，tabular-nums 等宽数字
...
const ParallelMeter = ({ stats }: { stats: ParallelStats }) => {
  ...
  return (
    <div className="flex items-center gap-4">
      <span>
        同块并发 <b className="text-[#a78bfa] text-2xl tabular-nums">{stats.concurrent}</b>
      </span>
      <span>
        整链 <b className="text-[#34d399] text-2xl tabular-nums">{stats.chainTx ?? "—"}</b>
        <span className="text-base text-[#94a3b8]"> tx/块</span>
      </span>
      ...
      <div className="flex items-end gap-[2px] h-8" title="本合约最近 20 块每块落子数">
        {stats.perBlock.map((n, i) => (
          <div
            key={i}
            className={`w-[4px] ${i === lastIdx ? "bg-[#a78bfa] animate-pulse" : "bg-[#a78bfa66]"}`}
