...
// 配色遵循 dataviz 规范：金=活动量 / 紫=结构 / 绿=性能（与 stage HUD 分类色语义一致），无渐变；色值统一走 globals.css 的 mp-* 设计令牌
...
const StatTile = ({ label, value, color }: { label: string; value: number; color: string }) => (
  <div className="py-6 rounded-xl bg-mp-surface border border-mp-border">
    <div className="text-7xl font-black tabular-nums" style={{ color }}>
      {value}
    </div>
    <div className="mt-2 text-lg text-mp-muted">{label}</div>
  </div>
);
    ...
    <>
      ...
      {phase === "idle" && (
        <button
          ...
          className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 px-6 py-3 rounded-full bg-mp-surface border border-mp-accent text-mp-accent text-xl font-bold shadow-[0_0_20px_color-mix(in_oklab,var(--mp-accent)_27%,transparent)] hover:bg-mp-accent/15 transition-colors cursor-pointer"
        >
          🎵 发起合唱
        </button>
      )}
      ...
      {phase !== "idle" && (
        <div
          className={`fixed inset-0 z-50 flex flex-col items-center justify-center text-center px-6 ${
            phase === "collecting" ? "bg-mp-bg/50" : "bg-mp-bg/85"
          }`}
        >
          {phase === "countdown" && (
            <div>
              <div
                key={countdown}
                className="animate-pulse text-[10rem] leading-none font-black text-mp-accent tabular-nums"
                style={{ textShadow: "0 0 60px color-mix(in oklab, var(--mp-accent) 40%, transparent)" }}
              >
                {countdown}
              </div>
              <p className="mt-4 text-2xl text-mp-fg">准备同时落子！</p>
            </div>
          )}
          ...
          {phase === "collecting" && (
            <div className="animate-pulse">
              ...
              <div className="mt-2 text-3xl text-mp-fg font-bold">全场合唱中</div>
              <div className="mt-2 text-8xl font-black text-mp-gold tabular-nums">{liveCount}</div>
              <p className="mt-1 text-xl text-mp-muted">笔交易正在并行执行…</p>
            </div>
          )}

          {phase === "settling" && <p className="text-3xl text-mp-muted animate-pulse">正在链上对账…</p>}

          {phase === "report" && report && (
            <div className="max-w-4xl w-full">
              <h2 className="text-4xl font-bold text-mp-fg">🎺 合唱战报</h2>
              <p className="mt-2 text-xl text-mp-muted">刚才 {CHORUS_WINDOW_MS / 1000} 秒 ——</p>

              {/* 三联指标：金=交易量 / 紫=区块结构 / 绿=并发性能 */}
              <div className="mt-6 grid grid-cols-3 gap-6">
                <StatTile label="笔交易" value={report.total} color="var(--mp-gold)" />
                <StatTile label="覆盖区块" value={report.blocks} color="var(--mp-accent)" />
                <StatTile label="单块最高并发" value={report.peakPerBlock} color="var(--mp-ok)" />
              </div>
              <p className="mt-4 text-lg text-mp-muted">{report.players} 名玩家同刻参战</p>

              {/* 以太坊对比为预写保守话术（02 方案：写死「数十分钟级」，不现场计算） */}
              <p className="mt-8 text-2xl text-mp-fg">
                在以太坊，这些交易串行需排队<span className="text-mp-gold font-bold">数十分钟级</span>； 在 Monad，它们
                <span className="text-mp-ok font-bold">并行完成了</span>
              </p>

              <div className="mt-10 flex gap-4 justify-center">
                <button
                  onClick={() => void startChorus()}
                  className="px-6 py-3 rounded-full bg-mp-surface border border-mp-accent text-mp-accent text-lg font-bold hover:bg-mp-accent/15 transition-colors cursor-pointer"
                >
                  🔁 再来一场
                </button>
                <button
                  onClick={() => setPhase("idle")}
                  className="px-6 py-3 rounded-full bg-mp-surface border border-mp-border text-mp-muted text-lg hover:bg-mp-accent/15 transition-colors cursor-pointer"
                >
                  ✕ 关闭
                </button>
              </div>
